...
from det3d.ops.points_op import pts_in_boxes3d
from .. import builder
from ..losses import accuracy
from ..registry import HEADS
from ..backbones.scn import USE_SA_SSD
...
def smooth_l1_losss(pred, target, beta=1.0, reduction='mean'):
    assert beta > 0
...
def weighted_smoothl1(pred, target, weight, beta=1.0, avg_factor=None):
    ...
    loss = smooth_l1_losss(pred, target, beta, reduction='none')
...
class MultiGroupHead(nn.Module):
    ...
    def build_aux_target(self, nxyz, gt_boxes3d, enlarge=1.0):
        ...
        for i in range(len(gt_boxes3d)):
            boxes3d =torch.from_numpy(gt_boxes3d[i])
            idx = torch.nonzero(nxyz[:, 0] == i).view(-1)
            new_xyz = nxyz[idx, 1:].cpu()

            boxes3d[:, 3:6] *= enlarge

            pts_in_flag, center_offset = pts_in_boxes3d(new_xyz, boxes3d.type_as(new_xyz))
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if USE_SA_SSD and self.training:
                point,point_cls,point_reg=example['point_mis']
                gt_box=np.concatenate(example['gt_boxes'],axis=1)
                aug_loss=self.aux_loss(point,point_cls,point_reg,gt_box)
                for k in aug_loss:
                    loss+=aug_loss[k].item()
            ret = {
                "loss": loss,
