...
def boxes3d_to_corners3d_camera(boxes3d, bottom_center=False):
    """
...
def get_corner_line(bbox_list):
    line = []
    for bbox in bbox_list:
        line.append(np.linspace(bbox[0],bbox[1],num=200))
        line.append(np.linspace(bbox[ 0], bbox[ 3], num=200))
        line.append(np.linspace(bbox[ 0], bbox[ 4], num=200))
        line.append(np.linspace(bbox[7], bbox[ 4], num=200))
        line.append(np.linspace(bbox[7], bbox[6], num=200))
        line.append( np.linspace(bbox[7], bbox[ 3], num=200))
        line.append(np.linspace(bbox[2], bbox[1], num=200))
        line.append(np.linspace(bbox[2], bbox[ 6], num=200))
        line.append(np.linspace(bbox[ 2], bbox[ 3], num=200))
        line.append(np.linspace(bbox[ 5], bbox[ 1], num=200))
        line.append(np.linspace(bbox[ 5], bbox[ 6], num=200))
        line.append(np.linspace(bbox[ 5], bbox[4], num=200))
    return  np.concatenate(line,axis=0)
...
class KittiDataset(PointCloudDataset):
    ...
    def convert_detection_to_kitti_annos(self, detection):
        ...
        for det_idx in gt_image_idxes:
            ...
            det['box3d_lidar'][:, 2] = (det['box3d_lidar'][:, 2].T +(h / 2).T).reshape(-1)
    ...
    def deeconvert_detection_to_kitti_annos(self, detection):
        class_names = self._class_names
        ...
        for det in detection:
            dim=det['dimensions']
            l, w, h = dim[:, 0:1], dim[:, 1:2], dim[:, 2:3]
            det['location'][:, 2] = (det['location'][:, 2].T +(h / 2).T).reshape(-1)
            det['rotation_y']= det['rotation_y']
            annos.append(det)


            # final_box_preds = det["box3d_lidar"]
            # label_preds = det["name"]
            # scores = det["scores"]
            # anno = get_start_result_anno()
            # num_example = 0
    ...
    def evaluation(self, detections, output_dir=None):
        ...
        # with open('/home/jty/data/kitti/deecamp/dee_infos_val.pkl', 'rb') as input_file:
        #     dt_annos = pickle.load(input_file)
        #
        # det_annos=gt_annos
        # det = []
        # for anno in det_annos:
        #     #anno['annos']['bbox'] = np.zeros([anno['annos']['bbox'].shape[0], 4])
        #     # anno['score']=np.ones([anno['name'].shape[0]])*0
        #     # anno['score'] = np.ones([anno['name'].shape[0]]) * 0
        #     det.append(anno)
        # dt_annos = det
        # gt_annos = dt_annos
        dt_annos=self.deeconvert_detection_to_kitti_annos(detections)
    ...
    def get_sensor_data(self, idx, with_image=False, with_gp=False):
        ...
        info['point_cloud']['velodyne_path']="/home/jty/data/kitti/deecamp/train_val_filter/" + info['point_cloud']['lidar_idx'].split('/')[-1]+'.bin'
